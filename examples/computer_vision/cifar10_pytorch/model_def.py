...
class CIFARTrial(DeepSpeedTrial):
    def __init__(self, context: PyTorchTrialContext) -> None:
        ...
        model_engine = deepspeed.initialize(
            args=AttrDict(self.context.get_hparams()),
            model=net,
    ...
    def evaluate_batch(self, iter_dataloader, batch_idx) -> Dict[str, Any]:
        ...
        accuracy = accuracy_rate(output, targets)
    ...
    def build_training_data_loader(self) -> Any:
        ...
        return DataLoader(trainset, batch_size=self.model.train_micro_batch_size_per_gpu())

    def build_validation_data_loader(self) -> Any:
        valset = self._download_dataset(train=False)
        return DataLoader(valset, batch_size=self.model.train_micro_batch_size_per_gpu())
